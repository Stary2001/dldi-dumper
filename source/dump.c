...
void relocate(uint32_t *dat, uint32_t len, uint32_t off, uint32_t a, uint32_t b)
{
...
//---------------------------------------------------------------------------------
	...
	if (fatInitDefault()) {
		iprintf("Finding DLDI signature.. (%08x?)", *(uint32_t*)0x02000b40);
		uint32_t *scan_start = (unsigned int*)0x02000000;
		uint32_t *scan_end =   (unsigned int*)0x02001000;
